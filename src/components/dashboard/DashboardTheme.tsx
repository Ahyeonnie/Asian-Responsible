...
export default function DashboardTheme() {
  ...
  useEffect(() => {
    ...
    if (saved === "corporate" || saved === "playful") {
      setCurrentTheme(saved);
    }
    
    // Load default theme mode preference
    const savedMode = localStorage.getItem("defaultThemeMode");
    if (savedMode === "light" || savedMode === "dark") {
      setDefaultThemeMode(savedMode);
    }
  }, []);

  const switchTheme = (theme: "playful" | "corporate") => {
    localStorage.setItem("designTheme", theme);
    setCurrentTheme(theme);
    toast.success(`Switched to ${theme === "corporate" ? "Corporate" : "Colorful & Playful"} Design!`);
    
    // Trigger a storage event for App.tsx to listen to
  ...
  const setDefaultMode = (mode: "light" | "dark") => {
    localStorage.setItem("defaultThemeMode", mode);
